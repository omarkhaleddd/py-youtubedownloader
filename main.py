...
sg.theme('Default')
start_layout = [[sg.Input('',key = '-INPUT-'),sg.Button('Download')]]
info_tab = [
    [sg.Text('Title :'),sg.Text('',key='-TITLE-')],
    [sg.Text('Length :'),sg.Text('',key='-LENGTH-')],
    [sg.Text('Views :'),sg.Text('',key='-VIEWS-')],
    [sg.Text('Authors :'),sg.Text('',key='-AUTHOR-')],
    [sg.Text('description :'),sg.Multiline('', key = '-DESCRIPTION-',size = (40,20), disabled = True)]
...
download_tab = [
    ...
    [sg.VPush()],
    [sg.Progress(100,orientation='h', size=(20, 20), key='-DOWNLOADPROGRESS-', expand_x = True,bar_color=('green','white'))]
]
program_layout = [[sg.TabGroup([[sg.Tab('info',info_tab),sg.Tab('download',download_tab)]])]]

window = sg.Window('Youtube Downloader',start_layout, icon='logo.ico')

video_link = 'https://www.youtube.com/watch?v=QeMaWQZllhg'
while True :
    ...
    if event == 'Download':
        ...
        window = sg.Window('Youtube Downloader',program_layout, icon='logo.ico', finalize= True)
        #video info
